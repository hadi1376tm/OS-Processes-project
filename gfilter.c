...
int main(int argc, char **argv) {
    ...
    char Output_addr[]="gaussian.jpg";
    sprintf(out_file, Output_addr);
    ...
    if(mkfifo("g_fifo", 0777)==-1){
        if(errno != EEXIST){
            printf("could not create fifo in gfilter\n");
            exit(1);
            }
        }
    int gf_fd= open("g_fifo", O_WRONLY);
    if(write(gf_fd,&Output_addr,strlen(&Output_addr))==-1){
        printf("could not write fifo in gfilter\n");
        exit(1);
    }
    printf("gaussian sent to C\n");
    close(gf_fd);
